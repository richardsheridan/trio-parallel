...
class SintWorker(_abc.AbstractWorker):
    ...
    async def start(self):
        ...
        send_cid = self._send_cid = channels.create()
        recv_cid = self._recv_cid = channels.create()
        ...
        def sint_thread_behavior():
            interpreters.run_string(interp, script, None)

        def sint_thread_deliver(result):
            result.unwrap()

        trio.lowlevel.start_thread_soon(sint_thread_behavior, sint_thread_deliver)
        channels.send(send_cid, self._init)
        channels.send(send_cid, self._retire)
        while (code := channels.recv(recv_cid, None)) is None:
    ...
    async def run_sync(self, sync_fn: Callable, *args) -> Optional[Outcome]:
        ...
        try:
            try:
                channels.send(self._send_cid, job)
            ...
            try:
                while (result := channels.recv(self._recv_cid, False)) is False:
    ...
    def shutdown(self):
        try:
            channels.close(self._send_cid, send=True)
...
WORKER_SINT_MAP = {"subinterpreter": (SintWorker, WorkerSintCache)}
